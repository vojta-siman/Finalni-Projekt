...
function vykresliKosik() {
    ...
    if (kosik.length === 0) {
        ...
        if (mezisoucetElement) mezisoucetElement.innerText = '0 Kč';
        if (marzeElement) marzeElement.innerText = '0 Kč'; // Vynulování marže při prázdném košíku
        if (celkemElement) celkemElement.innerText = '0 Kč';
        return;
    }
    ...
    kosik.forEach((produkt: any, index: number) => {
    ...
    if (mezisoucetElement) mezisoucetElement.innerText = `${mezisoucetCena} Kč`;
    if (marzeElement) marzeElement.innerText = `${hodnotaMarze} Kč`; // Zápis samotné marže v Kč
    if (celkemElement) celkemElement.innerText = `${konecnaCelkovaCena} Kč`;
}

/**
 * FUNKCE PRO ODEBRÁNÍ PRODUKTU Z KOŠÍKU
 */
(window as any).odebratZKosiku = function(indexProSmazani: number) {
